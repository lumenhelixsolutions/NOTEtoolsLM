...
function _detectCapabilities() {
  ...
  capabilities = {
    ...
    canListSources: typeof sdkClient.sources?.list === 'function'
  };
...
async function listNotebookSources(notebookId) {
  const client = await getSdkClient();
  if (typeof client.sources?.list !== 'function') {
    throw new Error('SDK sources.list method not available');
  }
...
function resetSdk() {
  sdkClient = null;
  sdkAuthed = false;
  capabilities = { connected: false, canListNotebooks: false, canListArtifacts: false, canCreateAudio: false, canCreateVideo: false, canCreateSlides: false, canCreateMindMap: false, canCreateReport: false, canDownload: false, canListSources: false };
}
